...
%%
  ndim  = [500, 1e3, 1e3, 1e3, 5e3, 5e3, 5e3, 1e4, 1e4, 1e4, 2e4, 3e4, 5e4, 1e5];
  rrank = [25, 10,  50,  100, 10,  50,  100, 10,  50,  100, 10,  10,  10,  10]; 
  pfac  = [2, 6,   4,   3,   6,   5,   4,   6,   5,   4,   6,   6,   6,   6];
%%
  problem_type = 'NNLS'; 
  %%problem_type = 'SDLS'; 
%   scenario = 'noiseless';  
...
%%
  for kk = [1]; 
      ...
      for randstate = [randstate_idx];   
         ...
         [X,iter,time,sd,hist] = APGL(nr,nc,problem_type,Amap,ATmap,bb,mutarget,0,par);
%          [X,iter,time,hist] = myAPGL1(nr, nc, Amap, ATmap, bb, mutarget, 1, 0.5, 1e-4);
%          [X,iter,time,hist] = myAPGL3(nr, nc, Amap, ATmap, bb, mutarget, 1, 0.5, 1e-4);
%          [X,iter,time,hist] = myAPG(nr, nc, Amap, ATmap, bb, mutarget, 1, 0.5, 1e-4);
%          [X,iter,time,hist] = myAPGL4(nr, nc, Amap, ATmap, bb, mutarget, 1, 0.5, 1e-4);
...
%          runhist.minsig(randstate) = min(sd(find(sd>0)));
        runhist.relDist(randstate) = hist.relDist(end);
        runhist.relRes(randstate) = hist.relRes(end);
        runhist.relObjdiff(randstate) = hist.relObjdiff(end);
         %%
...
%          fprintf('\n # singular         :  %6.0f',runhist.svp(randstate));
         ...
         fprintf('\n relative distance  :  %6.2e',runhist.relDist(randstate));
         fprintf('\n relative residual  :  %6.2e',runhist.relRes(randstate));
         fprintf('\n relative objective difference  :  %6.2e',runhist.relObjdiff(randstate));
         fprintf('\n------------------------------------------------'); 
         fprintf('------------------------------\n')
